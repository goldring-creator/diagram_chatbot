...
- 변수 인과·매개·구조방정식(SEM)·잠재변수·측정모형 → "path_model"
...
[출력 계약 — 아래 스키마의 JSON 하나만]
{
  ...
  "nodes": [
    {"id":"n1", "label":"짧은 라벨", "sub":"부제(선택)",
     ...
     "kind":"latent|observed (path_model의 SEM에서만: latent=잠재변수 타원, observed=관측지표)"}
  ],
  "edges": [
    {"from":"id", "to":"id", "style":"solid|dashed|measure", "label":"관계명·경로계수(선택)"}
  ],
...
- path_model: nodes(변수), edges(경로 방향. 주경로 solid, 보조·직접효과 dashed).
  구조방정식(SEM)·측정모형이면: 잠재변수는 "kind":"latent", 관측지표(설문 문항 등)는
  "kind":"observed"로 표시하고 잠재→지표 엣지에 "style":"measure"를 쓴다.
  경로계수·유의도가 문서에 있으면 edge의 label에 짧게 적는다(예: ".358***").
  문서에 없는 계수는 지어내지 않는다.
...
유형 변경 요청(예: "트리형으로 바꿔줘", "흐름도로")이면 diagram_type을 바꾸고
기존 내용을 새 유형의 필수 필드에 맞게 옮겨 담는다.
