...
```powershell
cd 소스코드
py -m pip install --user openai pypdf python-docx resvg-py   # 최초 1회
py app.py
```
...
```bash
cd "소스코드"
/usr/bin/python3 -m pip install --user openai pypdf python-docx resvg-py   # 최초 1회
...
- 입력칸: **Enter=전송, Shift+Enter=줄바꿈**
  (알려진 제약: 윈도우 한글 IME에서 조합 확정용 Enter가 드물게 전송으로 처리될 수 있다 —
  Tk의 IME 처리 한계로, 마지막 글자 확정 후 Enter를 한 박자 늦게 누르면 안전하다)
...
- `core.py` — 키·CJK검사·파일읽기(PDF·docx·HTML·txt)·OCR·`get_spec`(JSON 완결 응답)
